...
def is_domain(input_string):
    # Regex pattern to detect domain names with optional multi-level extensions
    domain_regex = r"^(?:[a-zA-Z0-9-]+\.)+[a-zA-Z]{2,}(?:\.[a-zA-Z]{2,})?$"
    
    # Check if the input string matches the domain pattern
    return bool(re.match(domain_regex, input_string))
